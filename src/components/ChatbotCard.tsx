...
import { sendMessageAction } from '../app/actions/chatAction';
  ...
  const [messages, setMessages] = useState<Message[]>([
    {
      id: 'welcome',
      role: 'model',
      content: "Hola. Soy tu asistente legal con IA. ¿Cómo puedo ayudarte a resolver tus dudas legales hoy?",
      timestamp: new Date()
    }
  ]);
  ...
  const handleSendMessage = async () => {
    ...
    try {
      ...
      setMessages(prev => [...prev, {
        id: Date.now().toString(),
        role: 'model',
        content: "Disculpe, pero tengo un problema de conexión. Inténtelo de nuevo más tarde.",
        timestamp: new Date()
      }]);
    }
  };
    ...
    <div className="glass-panel rounded-2xl overflow-hidden flex flex-col h-[500px] w-full max-w-lg mx-auto shadow-2xl shadow-green-900/20 border border-white/10 relative bg-[rgba(20,20,30,0.6)] backdrop-blur-xl">
      ...
      <div className="px-6 py-4 border-b border-white/5 flex items-center justify-between bg-white/5">
        <div className="flex items-center space-x-3">
            ...
            <div>
                <h3 className="text-sm font-semibold text-white tracking-wide">AI Legal</h3>
                <p className="text-xs text-slate-400">Asistente AI • Online</p>
            </div>
        </div>
          ...
          <div className="flex justify-start">
            <div className="flex gap-3">
                ...
                <div className="bg-white/5 p-4 rounded-2xl border border-white/5 flex items-center space-x-2">
                    <div className="w-2 h-2 bg-[var(--brand-primary)] rounded-full animate-bounce" style={{ animationDelay: '0ms' }}></div>
                    <div className="w-2 h-2 bg-[var(--brand-primary)] rounded-full animate-bounce" style={{ animationDelay: '150ms' }}></div>
                    <div className="w-2 h-2 bg-[var(--brand-primary)] rounded-full animate-bounce" style={{ animationDelay: '300ms' }}></div>
                </div>
            </div>
          </div>
        )}
      ...
      <div className="p-4 border-t border-white/5 bg-black/20">
        <div className="relative">
          <input
            ...
            disabled={loadingState === LoadingState.LOADING || loadingState === LoadingState.STREAMING}
          />
          <button
            onClick={handleSendMessage}
            disabled={!inputValue.trim() || loadingState === LoadingState.LOADING || loadingState === LoadingState.STREAMING}
        ...
        <p className="text-center text-[10px] text-slate-600 mt-2">
            Las respuestas de IA son sólo para fines informativos y no asesoramiento legal.
        </p>
      </div>
    </div>
  );
